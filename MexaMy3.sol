 ...
 */
contract TokenMX3 is Context, BEP20, Ownable, TokenRecover {
    ...
    uint256 public constant maxFee = 12; //suma de todos los fees
    uint256 public constant maxTokensBurned = 50 * 10**19;
    uint256 public maxTxAmount = 25000 * 10**9; //0.1%
    uint256 public maxBalance = 12500 * 10**9; //0.05
    uint256 public numTokensSellToAddToLiquidity = 62500 * 10**9; //0.25%
    ...
    */
     ...
     */
    function setAllFees(uint256 _taxFee, uint256 _liquidityFee, uint256 _devFee, uint256 _charityFee, uint256 _burnFee) external onlyOwner() {
        require(_taxFee.add(_liquidityFee).add(_devFee).add(_charityFee).add(_burnFee) <= maxFee);
        taxFee = _taxFee;
        liquidityFee = _liquidityFee;
        devFee = _devFee;
        charityFee = _charityFee;
        burnFee = _burnFee;
        emit NewFees(_taxFee, _liquidityFee, _devFee, charityFee, _burnFee);
    }
    ...
    */
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if(!_isExcludedFromMaxBalance[recipient]) {
            require(balanceOf(recipient).add(amount) <= maxBalance, "Recipient exceeds maxBalance");
        }
